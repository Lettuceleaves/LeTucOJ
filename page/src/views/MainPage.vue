<template>
  <div class="contest-parent">
    <!-- 标题栏 -->
    <div class="title-bar">
      <div class="title-item" :class="{ active: activeTab === 'list' }" @click="switchTab('list')">
        题单
      </div>
      ...
      <div class="title-item" :class="{ active: activeTab === 'rank' }" @click="switchTab('rank')">
        排行榜
      </div>
...
const activeTab = ref(
  ['list', 'contest', 'rank', 'user', 'profile'].includes(initialTab) ? initialTab : 'list',
)

/* ---------- 4. 异步组件 ---------- */
const AsyncProblemList = defineAsyncComponent(
  () => import(/* webpackChunkName: "tab-problem-list" */ './MainPages/ProblemList.vue'),
)
const AsyncContest = defineAsyncComponent(
  () => import(/* webpackChunkName: "tab-contest" */ './MainPages/ContestList.vue'),
)
...
const AsyncUser = defineAsyncComponent(
  () => import(/* webpackChunkName: "tab-user" */ './ManagePage.vue'),
)
