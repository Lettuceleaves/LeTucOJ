  ...
  <div class="problem-list-container">
    ...
    <div class="toolbar">
        ...
        <div ref="sortDrop" class="dropdown">
          ...
          <div v-if="showSortOptions" class="dropdown-menu" @mousedown.stop>
            <label class="dropdown-item">
              <input type="radio" value="problem_name" v-model="sortField" @change="fetchProblems" />
              按英文名
            </label>
            <label class="dropdown-item">
              <input type="radio" value="problem_nick_name" v-model="sortField" @change="fetchProblems" />
              按中文名
            </label>
            <label class="dropdown-item">
      ...
      <div class="toolbar-right">
        <button
          v-if="userInfo && (userInfo.role === 'MANAGER' || userInfo.role === 'ROOT')"
          class="btn"
    ...
    <ul class="problem-list">
      <li
        v-for="item in displayedProblems"
        :key="item.problemName"
        class="problem-item"
        :class="{ done: item.accepted }"
      >
        <div class="problem-item-content">
          <router-link :to="`/editor/${item.problemName}`" class="problem-link">
            <div class="title-line">
              <strong>{{ item.problemNickName || '(无中文名)' }}</strong>
          ...
          <div
            ...
            class="modify-link"
          >
            <router-link :to="`/form?name=${item.problemName}`">修改</router-link>
          </div>
        </div>
      </li>
    </ul>
...
const fetchProblems = async () => {
  try {
    const resolvedOrder = sortField.value || 'difficulty'

    let response
    if (searchKeyword.value.trim()) {
      // 使用搜索接口
      response = await apiService.practice.searchProblems(
      ...
      // 使用普通列表接口
      response = await apiService.practice.getProblemList(
        currentPage.value,
        pageSize,
        resolvedOrder,
      )
    ...
    if (json.code === '0' && Array.isArray(json.data.problemBriefList)) {
      allProblems.value = json.data.problemBriefList
